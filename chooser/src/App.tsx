  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <div className="topbar-actions">
          <a
            ...
            href="https://github.com/goznauk/dotfiles"
            target="_blank"
            rel="noreferrer"
            aria-label="View goznauk dotfiles on GitHub"
          >
            View on GitHub
          </a>
      ...
      <div className="page-sections">
        ...
        <section className="scroll-section config-section" id={sectionAnchorId(VIEW_IDS.CONFIGS)}>
          <section className="panel config-panel">
            ...
            <div
              ...
              role="tabpanel"
              aria-labelledby={`config-tab-${activeConfig}`}
            >
              ...
              <aside className="config-support-column" aria-label={`${configDefinitions[activeConfig].title} support`}>
                <ConfigBlockNavigator
                  activeBlockId={activeConfigBlockIds[activeConfig]}
                  blocks={configBlocks[activeConfig]}
                  configKey={activeConfig}
                ...
                />
                <section className="config-output-panel" aria-labelledby="config-output-heading">
                  <p className="section-label">Config output</p>
                  <h2 id="config-output-heading">{configDefinitions[activeConfig].path}</h2>
...
const ConfigBlockEditor = ({
  ...
  const activeBlockLineCount = configBlockLineCount(activeBlock);
  const editorRows = Math.min(16, Math.max(5, activeBlockLineCount + 2));
...
const configBlockLineCount = (block: ConfigBlock) => block.content.trimEnd().split(/\r?\n/).length || 1;

const ConfigBlockNavigator = ({
  activeBlockId,
  blocks,
  configKey,
  draggedBlock,
  ...
  return (
    <nav className="config-block-list" aria-labelledby={`config-block-list-heading-${configKey}`}>
      <div className="code-pane-header">
        <strong id={`config-block-list-heading-${configKey}`}>Blocks</strong>
        <span>
        ...
        >
          <div className="editor-block-header">
            <button
              ...
              aria-label={`Select ${block.title} block, ${configBlockLineCount(block)} lines, ${
            ...
            >
              ...
              <span className="block-card-meta">
                ...
                <span className="meta-chip">{configBlockLineCount(block)} lines</span>
...
const CommandPanel = ({
  ...
  return (
    <aside className="panel sticky-panel">
      ...
      {activeTarget.implemented && runInTmux && (
        <p className="muted compact-note">
          Tmux keeps failure output visible. Sudo may prompt again inside tmux on TTY-scoped systems.
        </p>
      )}
