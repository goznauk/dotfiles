...
// @ts-expect-error Node type declarations are intentionally omitted from chooser command tests.
...
assertIncludes(
  ...
  "primary command only runs outer sudo preflight when tmux is missing"
);
assertIncludes(defaultCommands.primary, "sudo apt install -y 'tmux'", "outer preflight only installs tmux");
assertIncludes(
  defaultCommands.primary,
  'sudo apt install -y "ca-certificates" "curl" "git" "tmux"',
  "tmux bootstrap installs all bootstrap tools inside the tmux TTY"
);
assertIncludes(defaultCommands.primary, 'runner="$(mktemp)"', "primary command creates a local runner script");
assertIncludes(
  defaultCommands.primary,
  "cat > \"$runner\" <<'DOTFILES_RUNNER'",
  "primary command writes runner script through a quoted heredoc"
);
assertIncludes(
  defaultCommands.primary,
  'session="dotfiles-$(date +%Y%m%d-%H%M%S)-$$"',
  "primary command uses a unique dotfiles tmux session name"
);
assertIncludes(
  defaultCommands.primary,
  'tmux new-session -s "$session"',
  "primary command starts a non-ambiguous tmux session"
);
assertIncludes(
  defaultCommands.primary,
  'tmux new-session -s "$session" "$runner"',
  "tmux receives only the runner path"
);
assertIncludes(defaultCommands.primary, "curl -fsSL", "primary command downloads installer");
assertIncludes(defaultCommands.primary, "set -Eeuo pipefail", "tmux bootstrap uses strict shell mode");
assertIncludes(defaultCommands.primary, "sudo -v", "tmux bootstrap refreshes sudo inside the tmux TTY");
assertIncludes(defaultCommands.primary, "mktemp", "tmux bootstrap downloads installer to a temp file");
assertIncludes(defaultCommands.primary, '-o "$installer"', "tmux bootstrap writes curl output to the temp file");
assertIncludes(defaultCommands.primary, 'bash "$installer"', "tmux bootstrap executes the downloaded installer");
assertIncludes(defaultCommands.primary, "Bootstrap command exited with status", "tmux bootstrap reports final status");
assertIncludes(
  defaultCommands.primary,
  "Setup completed successfully.",
  "tmux bootstrap holds the pane open on success"
);
assertIncludes(
  defaultCommands.primary,
  "Failed. Review the output above.",
  "tmux bootstrap holds the pane open on failure"
);
assertIncludes(defaultCommands.primary, "Press Enter to close", "tmux bootstrap waits before closing the pane");
assertNotIncludes(defaultCommands.primary, " | bash", "primary command does not pipe curl directly to bash");
assertNotIncludes(
  defaultCommands.primary,
  "tmux new-session -A -s dotfiles",
  "primary command does not attach to an old dotfiles session"
);
assertNotIncludes(defaultCommands.primary, "bash -lc", "primary command does not nest the full script in bash -lc");
assertNotIncludes(defaultCommands.primary, "; status=", "tmux command does not include a semicolon status chain");
assertIncludes(defaultCommands.primary, "/master/install.sh", "default command uses the published master branch");
assertNotIncludes(
  defaultCommands.primary,
  "/main/install.sh",
  "default command does not use an unpublished main branch"
);
assertNotIncludes(defaultCommands.primary, "DOTFILES_REPO_REF=", "default ref does not need env prefix");
execFileSync("bash", ["-n", "-c", defaultCommands.primary], { stdio: "pipe" });
const runnerScript = defaultCommands.primary
  .split("cat > \"$runner\" <<'DOTFILES_RUNNER'\n")[1]
  ?.split("\nDOTFILES_RUNNER")[0];
if (!runnerScript) {
  throw new Error(`runner script could not be extracted\nActual: ${defaultCommands.primary}`);
}
execFileSync("bash", ["-n"], { input: runnerScript, stdio: "pipe" });
