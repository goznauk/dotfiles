...
- Ubuntu install commands through `install.sh`, downloaded to a temporary file before execution rather than piped directly from `curl` to `bash`.
...
- Final install command and local command. The tmux bootstrap prints the installer exit status and keeps the pane open on failure; sudo may prompt once before tmux prerequisite installation and once inside tmux on TTY-scoped sudo configurations.
