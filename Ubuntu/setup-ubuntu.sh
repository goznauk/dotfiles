...
if (( BASH_VERSINFO[0] < 4 )) &&
  { [[ "${BASH_SOURCE[0]}" == "$0" ]] || [[ "${DOTFILES_TEST_ALLOW_LEGACY_BASH:-0}" != "1" ]]; }; then
...
ADMIN_USER_NAME=""
TARGET_VERSION=""
OH_MY_ZSH_URL="${DOTFILES_OH_MY_ZSH_URL:-https://github.com/ohmyzsh/ohmyzsh.git}"
POWERLEVEL10K_URL="${DOTFILES_POWERLEVEL10K_URL:-https://github.com/romkatv/powerlevel10k.git}"
ZSH_SYNTAX_HIGHLIGHTING_URL="${DOTFILES_ZSH_SYNTAX_HIGHLIGHTING_URL:-https://github.com/zsh-users/zsh-syntax-highlighting.git}"
ZSH_AUTOSUGGESTIONS_URL="${DOTFILES_ZSH_AUTOSUGGESTIONS_URL:-https://github.com/zsh-users/zsh-autosuggestions.git}"
ZSH_COMPLETIONS_URL="${DOTFILES_ZSH_COMPLETIONS_URL:-https://github.com/zsh-users/zsh-completions.git}"
...
clone_or_update() {
  ...
  if [[ -d "$dest/.git" ]]; then
    git_top="$(git -C "$dest" rev-parse --show-toplevel 2>/dev/null || true)"
    if [[ "$git_top" != "$dest" ]]; then
      warn "$dest is not a valid standalone git checkout."
      if [[ "$YES" -eq 1 ]]; then
        backup_path "$dest"
      ...
      warn "Move it aside and rerun setup to clone $url."
      return 0
    fi

    current_url="$(git -C "$dest" remote get-url origin 2>/dev/null || true)"
    if [[ -n "$current_url" && "${current_url%.git}" != "${url%.git}" ]]; then
      warn "$dest origin does not match $url."
      warn "Current origin: $current_url"
      if [[ "$YES" -eq 1 ]]; then
        backup_path "$dest"
        git clone --depth=1 "$url" "$dest"
        return 0
    ...
    warn "$dest already exists but is not a git checkout."
    if [[ "$YES" -eq 1 ]]; then
      backup_path "$dest"
      git clone --depth=1 "$url" "$dest"
      return 0
...
repair_managed_path() {
  ...
  warn "$reason"
  if [[ "$YES" -eq 1 ]] || confirm "Back up and recreate $path?"; then
    backup_path "$path"
    return 0
  fi

  printf 'Cannot continue while %s is broken. Move it aside or rerun with --yes to repair automatically.\n' "$path" >&2
  return 1
}
...
ensure_oh_my_zsh() {
  ...
  if [[ -d "$zsh_dir/.git" ]]; then
    git_top="$(git -C "$zsh_dir" rev-parse --show-toplevel 2>/dev/null || true)"
    current_url="$(git -C "$zsh_dir" remote get-url origin 2>/dev/null || true)"

    if [[ "$git_top" == "$zsh_dir" &&
      -n "$current_url" &&
      "${current_url%.git}" == "${OH_MY_ZSH_URL%.git}" ]]; then
      ...
      reason="$zsh_dir is missing required Oh My Zsh file oh-my-zsh.sh."
    elif [[ "$git_top" != "$zsh_dir" ]]; then
      reason="$zsh_dir is not a valid standalone Oh My Zsh checkout."
    else
      reason="$zsh_dir origin does not match $OH_MY_ZSH_URL."
      if [[ -n "$current_url" ]]; then
        reason="$reason Current origin: $current_url."
      fi
    fi
  elif [[ -e "$zsh_dir" || -L "$zsh_dir" ]]; then
    reason="$zsh_dir already exists but is not an Oh My Zsh git checkout."
  fi

  if [[ -n "${reason:-}" ]]; then
    repair_managed_path "$zsh_dir" "$reason"
  fi

  if [[ ! -e "$zsh_dir" && ! -L "$zsh_dir" ]]; then
    git clone --depth=1 "$OH_MY_ZSH_URL" "$zsh_dir"
  fi

  if [[ ! -f "$zsh_dir/oh-my-zsh.sh" ]]; then
    printf 'Oh My Zsh install is incomplete: %s is missing.\n' "$zsh_dir/oh-my-zsh.sh" >&2
    return 1
  fi
}
...
verify_zsh_setup() {
  ...
  log 'Verifying zsh setup'

  if [[ ! -f "$zsh_dir/oh-my-zsh.sh" ]]; then
    printf 'Oh My Zsh core is missing at %s.\n' "$zsh_dir/oh-my-zsh.sh" >&2
    return 1
  fi

  if [[ "$POWERLEVEL10K" -eq 1 &&
    ! -f "$zsh_custom/themes/powerlevel10k/powerlevel10k.zsh-theme" ]]; then
    printf 'Powerlevel10k theme is missing at %s.\n' "$zsh_custom/themes/powerlevel10k/powerlevel10k.zsh-theme" >&2
    return 1
  fi

  if [[ ! -f "$zsh_custom/plugins/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh" ]]; then
    printf 'zsh-syntax-highlighting is missing at %s.\n' "$zsh_custom/plugins/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh" >&2
    return 1
  fi

  if [[ ! -f "$zsh_custom/plugins/zsh-autosuggestions/zsh-autosuggestions.zsh" ]]; then
    printf 'zsh-autosuggestions is missing at %s.\n' "$zsh_custom/plugins/zsh-autosuggestions/zsh-autosuggestions.zsh" >&2
    return 1
  fi

  if [[ ! -d "$zsh_custom/plugins/zsh-completions/src" ]]; then
    printf 'zsh-completions source directory is missing at %s.\n' "$zsh_custom/plugins/zsh-completions/src" >&2
    return 1
  fi

  if [[ "$POWERLEVEL10K" -eq 1 && -r "$HOME/.zshrc" ]]; then
    if command -v zsh >/dev/null 2>&1; then
      if ! POWERLEVEL9K_DISABLE_CONFIGURATION_WIZARD=true \
        POWERLEVEL9K_INSTANT_PROMPT=off \
        HOME="$HOME" \
        ZSH="$zsh_dir" \
        ZSH_CUSTOM="$zsh_custom" \
        zsh -i -c 'type p10k >/dev/null'; then
        printf 'Powerlevel10k did not load from %s. Check Oh My Zsh and theme installation.\n' "$HOME/.zshrc" >&2
        return 1
      fi
    else
      warn 'zsh not found; skipping p10k load verification.'
    fi
  fi
}
...
install_shell() {
  ...
  if [[ "$POWERLEVEL10K" -eq 1 ]]; then
    clone_or_update "$POWERLEVEL10K_URL" "$zsh_custom/themes/powerlevel10k"
  fi
  clone_or_update "$ZSH_SYNTAX_HIGHLIGHTING_URL" "$zsh_custom/plugins/zsh-syntax-highlighting"
  clone_or_update "$ZSH_AUTOSUGGESTIONS_URL" "$zsh_custom/plugins/zsh-autosuggestions"
  clone_or_update "$ZSH_COMPLETIONS_URL" "$zsh_custom/plugins/zsh-completions"
  ...
  if command -v zsh >/dev/null 2>&1 && [[ "${SHELL:-}" != "$(command -v zsh)" ]]; then
    if [[ "$YES" -eq 1 ]]; then
      warn "Skipping login shell change in --yes mode to avoid an interactive chsh password prompt."
      warn "Run manually after setup if desired: chsh -s $(command -v zsh)"
    elif confirm 'Set zsh as the login shell?'; then
...
install_rust() {
  ...
  if ! rustup_executable >/dev/null 2>&1; then
    curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y --profile default
  fi

  if ! rustup_executable >/dev/null 2>&1; then
    warn 'rustup was not found after install.'
    return 1
  fi

  if ! run_rustup default stable; then
    warn 'Rust stable toolchain could not be selected; attempting repair.'
    repair_rust_stable_toolchain
  elif ! verify_rust_stable_toolchain; then
    warn 'Rust stable toolchain failed verification; attempting repair.'
    repair_rust_stable_toolchain
  fi

  run_rustup component add rustfmt clippy
}
...
run_rustup() {
  local rustup_cmd
  rustup_cmd="$(rustup_executable)" || return 1
  "$rustup_cmd" "$@"
}

run_rust_tool() {
  local tool="$1"
  local tool_path="$HOME/.cargo/bin/$tool"
  shift
...
verify_rust_stable_toolchain() {
  run_rustup --version >/dev/null || return 1
  run_rustup toolchain list >/dev/null || return 1
  run_rust_tool rustc --version >/dev/null || return 1
  run_rust_tool cargo --version >/dev/null || return 1
}

remove_stable_toolchain_dirs() {
  local rustup_home="${RUSTUP_HOME:-$HOME/.rustup}"
  local toolchains_dir="$rustup_home/toolchains"
  local path
  local removed=0

  for path in "$toolchains_dir"/stable-*; do
    [[ -e "$path" || -L "$path" ]] || continue
    case "$path" in
      "$toolchains_dir"/stable-*)
        rm -rf "$path"
        removed=1
        ;;
    esac
...
repair_rust_stable_toolchain() {
  ...
  warn 'Repairing Rust stable toolchain.'
  if ! run_rustup toolchain uninstall stable; then
    warn 'rustup could not uninstall stable; removing stable toolchain directories only.'
    remove_stable_toolchain_dirs || warn "No stable toolchain directories found under $rustup_home/toolchains."
  fi

  run_rustup toolchain install stable
  run_rustup default stable

  if ! verify_rust_stable_toolchain; then
    printf 'Rust stable toolchain repair failed. Inspect stable toolchain directories under %s/toolchains.\n' "$rustup_home" >&2
    return 1
  fi
}
...
install_mise() {
  ...
  if [[ "${MISE_READY:-0}" -eq 1 ]] && mise_executable >/dev/null 2>&1; then
    return 0
  fi
  ...
  if mise_executable >/dev/null 2>&1; then
    MISE_READY=1
    return 0
  fi
...
run_mise() {
  ...
  mise_cmd="$(mise_executable)" || return 1
  "$mise_cmd" "$@"
}
...
mise_enable_idiomatic_version_file() {
  ...
  current_settings="$(run_mise settings get idiomatic_version_file_enable_tools 2>/dev/null || true)"
  if [[ "$current_settings" == *"$tool"* ]]; then
    return 0
  fi

  run_mise settings add idiomatic_version_file_enable_tools "$tool" >/dev/null 2>&1 || true
}

install_mise_and_node() {
  log 'Installing Node LTS through mise'
  if ! run_mise use -g --pin node@lts; then
    warn 'Node LTS setup through mise failed.'
    return 1
  fi

  run_mise reshim node >/dev/null 2>&1 || true
...
verify_node_runtime() {
  run_node_command node --version >/dev/null 2>&1
}

verify_npm_runtime() {
  run_node_command npm --version >/dev/null 2>&1
}

repair_mise_node() {
  warn 'Repairing mise Node LTS install.'
  run_mise install --force node@lts
  run_mise use -g node@lts
  run_mise reshim node >/dev/null 2>&1 || true
}
...
ensure_npm_available() {
  ...
  if verify_node_runtime && verify_npm_runtime; then
    return 0
  fi

  if [[ "$NODE_STRATEGY" == "mise" ]]; then
    warn 'Node or npm failed verification after mise setup.'
    repair_mise_node || true
  fi
  ...
  printf 'Node was installed through mise but npm is not available; cannot install npm-based agent CLIs.\n' >&2
  return 1
}

install_package_manager_with_npm() {
  local package_manager="$1"

  ensure_npm_available || return 1
  log "Installing $package_manager through npm fallback"
  run_node_command npm install -g "$package_manager"
  if [[ "$NODE_STRATEGY" == "mise" ]]; then
    run_mise reshim node >/dev/null 2>&1 || true
  fi
  run_node_command "$package_manager" --version >/dev/null
}
...
install_node_package_manager() {
  ...
  case "$NODE_PACKAGE_MANAGER" in
    pnpm)
      ...
      if run_node_command corepack enable pnpm && run_node_command corepack prepare pnpm@latest --activate; then
        if [[ "$NODE_STRATEGY" == "mise" ]]; then
          run_mise reshim node >/dev/null 2>&1 || true
        fi
        run_node_command pnpm --version >/dev/null || install_package_manager_with_npm pnpm
      else
        warn 'Could not enable pnpm through Corepack; falling back to npm.'
        install_package_manager_with_npm pnpm
      fi
      ;;
    yarn)
      log 'Enabling Yarn through Corepack'
      if run_node_command corepack enable yarn && run_node_command corepack prepare yarn@stable --activate; then
        if [[ "$NODE_STRATEGY" == "mise" ]]; then
          run_mise reshim node >/dev/null 2>&1 || true
        fi
        run_node_command yarn --version >/dev/null || install_package_manager_with_npm yarn
      else
        warn 'Could not enable Yarn through Corepack; falling back to npm.'
        install_package_manager_with_npm yarn
      fi
      ;;
    npm)
...
install_agent_tools() {
  ...
  install_global_npm_packages "${packages[@]}"
}
