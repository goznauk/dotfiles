...
The chooser-generated remote Ubuntu command follows the same tmux-first workflow but adds a more defensive bootstrap: it downloads `install.sh` to a temporary file, runs that file with the selected flags, prints the installer exit status, and keeps the tmux pane open on failure. On a fresh host, sudo may ask once before tmux is installed and again inside tmux on systems where sudo authentication is scoped to each TTY.
...
For remote bootstraps, the chooser no longer pipes `curl` directly into `bash`. It downloads the installer with `curl -fsSL ... -o "$tmp"`, runs `bash "$tmp" ...`, and cleans up the temporary file after the run.
...
- Verifies `rustc` and `cargo` before adding components.
- Reinstalls only the stable toolchain if an existing stable rustup toolchain is corrupt.
...
- Default package manager: `pnpm` through Corepack, with npm fallback if Corepack is unavailable.
- Other package manager choices: `yarn` or `npm`.
- Node and npm are verified before package-manager setup or npm-based agent CLI installation.
